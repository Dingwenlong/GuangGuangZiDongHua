  ...
  <div class="mb-15 flex flex-row items-center flex-wrap gap-10">
    <div class="w-full flex justify-between flex-row items-center gap-10">
      <Input
        class="w-6/12!"
        readonly
        v-model:value="s6.taskDirectory"
        placeholder="点击选择任务监听目录文件夹"
        @click="selectDirectoryHandler"
        :disabled="s6.taskDirectory !== ''" />
      <div
        ...
        <template v-else-if="column.key === 'action'">
          <span>
            <Button
              type="primary"
              @click="
                openFolder(s6.taskDirectory + '\\' + record.productDirectory)
              "
              >打开文件夹</Button
            >
          </span>
...
async function selectDirectoryHandler() {
  s6.value.taskDirectory = await ipcRendererChannel.SelectDirectory.invoke();
}
...
function startOrStopTaskHandler(start = true) {
  if (start && s6.value.taskDirectory && !videoMonitoringRunning.value) {
    videoMonitoringRunning.value = true;
    ipcRendererChannel.StartMonitoringVideo.invoke(s6.value.taskDirectory);
...
onMounted(async () => {
  ...
  s6.value.taskDirectory = workbench.taskDirectory ?? '';
  s6.value.autoMonitoring = workbench.autoMonitoring ?? true;
  ...
  ipcRendererChannel.MonitoringDirectoryCallback.on(
    (event, arg: { root: string; structure: any[] }) => {
      data.value = arg.structure
        ...
        .map(dir => {
          return {
            taskDirectory: s6.value.taskDirectory,
            productDirectory: dir.path.replace(
              s6.value.taskDirectory + '\\',
              ''
            ),
