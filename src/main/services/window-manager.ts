...
import { registerIpcHandlers, unregisterIpcHandlers } from "@main/services/ipc-main";
import { ipcCustomLoginHandlers, ipcCustomMainHandlers } from "@main/services/ipc-custom-handle";
...
class MainInit {
  ...
  async createLoginWindow() {
    if(config.IgnoreLogin || await authManager.isLoggedIn()) return await this.createMainWindow();
